...
public:
    ...
    int findPaths(int x, int y, vector<vector<int>>& obstacleGrid,  vector<vector<int>>& dp){
        ...
        return dp[x][y] = findPaths(x-1,y,obstacleGrid,dp) + findPaths(x,y-1,obstacleGrid,dp);
    }

    int uniquePathsWithObstacles(vector<vector<int>>& obstacleGrid) {
        
        vector<vector<int>> dp(obstacleGrid.size(),vector<int>(obstacleGrid[0].size(),-1));
        return findPaths(obstacleGrid.size()-1,obstacleGrid[0].size()-1,obstacleGrid,dp);
    }
};